...
const BASE_PATH = "./circuits/CircuitSangam/";
...
async function generateProof() {
  ...
  const proof = await snarkjs.groth16.prove(
    BASE_PATH + "out/CircuitSangam.zkey",
    BASE_PATH + "out/circuit.wtns"
  )
...
async function main() {
  // deploy contract
  const Verifier = await ethers.getContractFactory("./contracts/CircuitSangamVerifier.sol:Verifier");
